...
const PAGE_SIZE = 50; // Number of clients to load per page

const ClientList = () => {
  const [allData] = useState<ClientRecord[]>(clientlistdetails);
  const [visibleData, setVisibleData] = useState<ClientRecord[]>(
  ...
  const columns = [
    {
      title: "Client ID",
      dataIndex: "ClientID",
      render: (_text: string, record: ClientRecord) => (
        <Link to={all_routes.clientdetils}>{record.ClientID}</Link>
      ),
      sorter: (a: ClientRecord, b: ClientRecord) =>
        a.ClientID.length - b.ClientID.length,
    },
    {
      title: "Client Name",
